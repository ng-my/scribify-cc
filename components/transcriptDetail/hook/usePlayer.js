...
export default function usePlayer(transcriptData, isShowVideo, props) {
  ...
  // 初始化播放器
  const initPlayers = (fileBaseInfo, locale, allSegments, bindShortEvent) => {
    if (isVideo.value) {
  ...
  function isChildInParentViewport(parent, child, partiallyVisible = true) {
  ...
  const _scrollToActiveWord = () => {
    try {
      // 早期返回条件检查
      if (!transcriptData.value?.paragraphs?.length || isScrolling) {
        return;
      }
      ...
      const { paraIndex, sentIndex } = result;
      if (!result) {
        return;
      }
  ...
  const setAc = (paraIndex, sentIndex, contentIndex, content, sentenceId) => {
    lastSentenceId.value = sentenceId;
    // 设置当前活动单词
    activeWord.value = { paraIndex, sentIndex, contentIndex, content };
  ...
  const handleWordClick = async (
    ...
    clickTimer = setTimeout(() => {
      setAc(paraIndex, sentIndex, contentIndex, content, sentenceId);
    }, 250);
  };
  // 双击单词
  const handleWordDblClick = (
    paraIndex,
    sentIndex,
    contentIndex,
    content,
    ...
    setTimeout(() => {
      player.seek(+content.start_time, "play");
    }, 100);
  };
  ...
  // 判断单词是否应该高亮
  const isWordActive = (content, paraIndex) => {
    // 用户点击的高亮
    const isClickedActive =
      activeWord.value.paraIndex === paraIndex &&
      activeWord.value.content?.cid === content.cid;
    if (isClickedActive) {
      return true;
    }
    // 当前播放时间的高亮
    ...
    return isTimeActive && activeWord.value.paraIndex === -1;
  };
