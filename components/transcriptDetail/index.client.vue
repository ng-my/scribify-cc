          ...
          <div class="file-wrap flex-1 overflow-hidden">
            <div class="file-name line-clamp-1 break-all leading-5 text-black">
              <span
                ...
                @click="startEditing"
                ref="fileNameElement"
                class="file-text hover-effect cursor-pointer text-base font-medium"
                :title="fileName"
              >
                {{ fileName }}
              </span>
              ...
              <el-popover
                ...
                popper-class="filename-pop !rounded-lg !border-0"
                trigger="click"
                @hide="saveFileName"
              >
                <el-input
            ...
            >
              ...
              >
                ...
                <el-button
                  ...
                  @click="
                    handleSaveRightTranscript(() => (lastSentenceId = ''))
                  "
                >
                          ...
                          >
                            ...
                            <span
                              ...
                              :class="[
                                ...
                                {
                                  'transition-colors md:hover:bg-hoverColor-deepen md:hover:text-black':
                                    !isEditRightTranscript
                                },
                                isWordActive(word, item.pid) &&
                                (!isEditRightTranscript ||
                                  (isEditRightTranscript &&
                                    !isActiveEditingSentence(sentence.sid) &&
                                    (lastSentenceId === '' ||
                                      lastSentenceId !== sentence.sid)))
                                  ? 'bg-hoverColor-deepen text-black transition-colors'
                                  : ''
                              ]"
                              ...
                              @dblclick="
                                ...
                                handleWordDblClick(
                                  ...
                                  wordIndex,
                                  word,
                                  isEditRightTranscript ? sentence.sid : ''
                                )
                              "
                              @click="
                                handleWordClick(
...
watch([langId, isShowSpeaker, isShowTimestamp, isShowVideo], () => {
  emit("saveConfig");
});
// 分享相关
...
const genShareImage = () => {
  ...
  const text = `"${props.shareName}" ${t("TranscriptionPage.shared")}`;
};
// 生命周期钩子
onMounted(async () => {
  await nextTick();
